...
// COMMENTS TO GRADER:
// In task 1, to control the termination of iterations, I use an variable called "threshold", when the unshot power
// in the current iteration is less than the threshold * maximum value of unshot power, the iteration will terminate.

// In task 2, you need to change the input model file to "new_model.in" and change output file to "new_model.out" 
//
// ============================================================
//
...
// In the iteration, if unshotpower / max value of unshotpower < threshold, it will terminate
    ...
    // Note that numPixelsOnWidth must be a even number.
{
     ...
     ****************** WRITE YOUR CODE HERE ******************
     **********************************************************/
    float deltaA = 1.0 / pow(numPixelsOnWidth / 2.0, 2);


    for(int i = 0; i < numPixelsOnWidth; i++)
        for(int j = 0; j < numPixelsOnWidth; j++)
        {
            float x = -1.0 + 1.0 / numPixelsOnWidth + (float)i * 2.0 / numPixelsOnWidth; 
            float y = -1.0 + 1.0 / numPixelsOnWidth + (float)j * 2.0 / numPixelsOnWidth;
            deltaFormFactors[i * numPixelsOnWidth + j] = deltaA / (M_PI * pow(pow(x, 2) + pow(y, 2) + 1.0, 2));
        }
}
    ...
    // Note that numPixelsOnWidth must be a even number.
{
    ...
    for(int i = 0; i < numPixelsOnWidth / 2; i++)
        for(int j = 0; j < numPixelsOnWidth; j++)
        {
            float z = 1.0 / numPixelsOnWidth + (float)i * 2.0 / numPixelsOnWidth;
            float y = -1.0 + 1.0 / numPixelsOnWidth + (float)j * 2.0 / numPixelsOnWidth;
            deltaFormFactors[i * numPixelsOnWidth + j] = deltaA * z / (M_PI * pow(pow(y, 2) + pow(z, 2) + 1.0, 2));
        }
    ...
    // Need to set up the viewport, projection and view transfromation.
{
    ...
    // For top view of hemicube ,window size is as before
    ...
    // up vector of glLookAt
    float lookUpVector[3];

    // edgeVector is from the first vertex of polygon points to the second
    float edgeVector[3];
    VecDiff(edgeVector, shooterQuad->v[0], shooterQuad->v[1]);
    
    // up vector is vertical to edge vector
    VecCrossProd(lookUpVector, shooterQuad->normal, edgeVector);

    float lookAt[3];
    for(int i = 0; i < 3; i++)
        lookAt[i] = shooterQuad->centroid[i] + shooterQuad->normal[i];
    ...
    // Need to set up the viewport, projection and view transfromation.
{
    ...
    // For top view of hemicube ,window size is as half as before
    ...
    if (face == 1)
        for(int i = 0; i < 3; i++)
            lookDirection[i] = edgeVector[i];
    else if (face == 2)
        VecCrossProd(lookDirection, shooterQuad->normal, edgeVector);
    else if (face == 3)
        VecNeg(lookDirection, edgeVector);
    else if (face == 4){
        VecCrossProd(lookDirection, shooterQuad->normal, edgeVector);
        VecNeg(lookDirection, lookDirection);
    }

    for(int i = 0; i < 3; i++)
        lookAt[i] = shooterQuad->centroid[i] + lookDirection[i];
    ...
    // and update the unshot power of their parent shooter quads.
{
    for ( int i = 0; i < width * height; i++ )
    {
        ...
        // For each channel, update the radiosity of gathering patch and unshot power of shooter patch
        m->gatherers[g]->radiosity[0] += m->gatherers[g]->surface->reflectivity[0] * deltaFormFactors[i] * shotPower[0] / m->gatherers[g]->area;
        m->gatherers[g]->radiosity[1] += m->gatherers[g]->surface->reflectivity[1] * deltaFormFactors[i] * shotPower[1] / m->gatherers[g]->area;
        m->gatherers[g]->radiosity[2] += m->gatherers[g]->surface->reflectivity[2] * deltaFormFactors[i] * shotPower[2] / m->gatherers[g]->area;
...
static void ComputeRadiosity( void )
{
    ...
    // Find a shooter quad to shoot power.
		...
		// Record the max value of unshot power
        static float unshotPowerMax[3] = {0};
        for(int i = 0; i < 3; i++)
            unshotPowerMax[i] = unshotPower[i] > unshotPowerMax[i] ? unshotPower[i] : unshotPowerMax[i];

        // In the iteration, for every channel
        // if unshotpower / max value of unshotpower < threshold, it will terminate
        if(
            unshotPower[0] / unshotPowerMax[0] < threshold && 
            unshotPower[1] / unshotPowerMax[1] < threshold && 
            unshotPower[2] / unshotPowerMax[2] < threshold 
        )
            break;
